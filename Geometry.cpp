...
Vector3 Matrix4::operator*(const Vector3 & v)
{
	...
	rv.e[0] = e[0] * v.e[0] + e[1] * v.e[1] + e[2] * v.e[2];
	rv.e[1] = e[4] * v.e[0] + e[5] * v.e[1] + e[6] * v.e[2];
	rv.e[2] = e[8] * v.e[0] + e[9] * v.e[1] + e[10] * v.e[2];
...
Matrix4 Matrix4::rotationMatrix(const float & yaw, const float & pitch, const float & roll)
{
	float cy = cos(yaw);
	float sy = sin(yaw);
	float cp = cos(pitch);
	...
	rm.e[0] = cp * cy; rm.e[1] = cy * sp * sr - cr * sy; rm.e[2] = cr * cy * sp + sr * sy; rm.e[3] = 0.0;
	rm.e[4] = cp * sy; rm.e[5] = cr * cy + sp * sr * sy; rm.e[6] = cr * sp * sy - cy * sr; rm.e[7] = 0.0;
	rm.e[8] = -sp; rm.e[9] = cp * sr; rm.e[10] = cp * cr; rm.e[11] = 0.0;
	rm.e[12] = rm.e[13] = rm.e[14] = 0.0; rm.e[15] = 1.0;
